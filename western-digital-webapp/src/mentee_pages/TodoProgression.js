...
function TodoProgression() {
  ...
  const user = JSON.parse(sessionStorage.getItem("user")) || { name: "Mentee", menteekey: 9999 };
  ...
  const [mockNotes, setMockNotes] = useState([]); // local storage for submitted reports
  // --------------------------------

  useEffect(() => {
    // Simulate loading meetings for this mentee (mock)
    const userInfo = JSON.parse(sessionStorage.getItem("user")) || { menteekey: 9999 };
    const menteeKey = userInfo?.menteekey;

    if (menteeKey) {
      // simulate network delay
      setTimeout(() => setMeetings(MOCK_MEETINGS), 300);
    } else {
  ...
  const handleAddReport = async (e) => {
    ...
    // Simulate a successful save locally (replaces POST)
    const newReportData = {
    ...
    // "Persist" to component state (mock DB)
    setMockNotes((prev) => [...prev, newReportData]);
          ...
          <div className="box">
            <div className="main-content">
              ...
              <div className="form-container">
                ...
                {/* Optional: show last 1–2 saved notes (mock DB) for quick feedback; won't affect UI layout */}
                {mockNotes.length > 0 && (
                  <div style={{ marginTop: "1rem", fontSize: "0.9rem" }}>
                    <strong>Saved reports (mock):</strong>
                    <ul>
                      {mockNotes.slice(-2).map((n) => (
                        <li key={n.id}>
                          {new Date(n.createdAt).toLocaleString()} — meeting {n.meetingkey}
                        </li>
                      ))}
                    </ul>
                  </div>
