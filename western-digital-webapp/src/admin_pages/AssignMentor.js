...
function AssignMentor() {
  ...
  const fetchMentorNames = async () => {
    setTimeout(() => {
      setMentors([
        { userid: "m1", name: "Alice", lastname: "Smith" },
        { userid: "m2", name: "Bob", lastname: "Johnson" },
      ]);
    }, 300);
  };
  ...
  const fetchMenteeNames = async () => {
    setTimeout(() => {
      setMenteesList([
        { userid: "u1", name: "Charlie", lastname: "Lee" },
        { userid: "u2", name: "Dana", lastname: "Brown" },
      ]);
    }, 300);
  };
  ...
  const handleAssignMentor = () => {
    if (newMentee && newMentor) {
      const newAssignment = {
        ...
        mentor_lastname: newMentor.lastname,
        mentee_name: newMentee.name,
        mentee_lastname: newMentee.lastname,
      };
      setRelationships((prev) => [...prev, newAssignment]);
      setNewMentee(null);
      setNewMentor(null);
      setErrorMessage("");
    } else {
  ...
  const handleUpdateMentor = (menteekey, newMentorkey) => {
    const selectedMentor = mentors.find((m) => m.userid === newMentorkey);
    if (!selectedMentor) return;
    setRelationships((prev) =>
      prev.map((rel) =>
        rel.menteekey === menteekey
          ? {
              ...rel,
              mentorkey: newMentorkey,
              mentor_name: selectedMentor.name,
              mentor_lastname: selectedMentor.lastname,
            }
          : rel
      )
    );
  ...
  const handleDeleteAssignment = (relationship_id) => {
    setRelationships((prev) =>
      prev.filter((rel) => rel.relationship_id !== relationship_id)
    );
  };
